...
class Tests(unittest.TestCase):
    ...
    def test_empty_dataframe(self):
        self.assertRaises(EmptyDataframeException,read_csv,Tests.path+"Novals.csv")
    
    #Test exception when the csv-file is empty:
    def test_emptyfile(self):
        self.assertRaises(pd.errors.EmptyDataError,read_csv,Tests.path+"empty.csv")
    
    #Check that corrupted values in date column raises exception
    def test_corrupted_date(self):
        self.assertRaises(ValueError,read_csv,Tests.path +"corrupted_date.csv")
        
    #Check that corrupted values in date column raises exception
    def test_corrupted_value(self):
        self.assertRaises(ValueError,read_csv,Tests.path+"corrupted_kurs.csv")
